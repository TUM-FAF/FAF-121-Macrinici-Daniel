...
int WINAPI WinMain(HINSTANCE hInst,HINSTANCE hPrevInst,LPSTR lpCmdLine,int nShowCmd) {
    ...
    HWND hWnd=CreateWindowEx(NULL,
            "Window Class",
            "Laboratory Work #3 Daniel Macrinici",
            WS_OVERLAPPEDWINDOW,
            250, 100,
            800, 600,
            NULL,
...
LRESULT CALLBACK WinProc(HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam) {
    ...
    SetRect(&rcGradient, 30, 370, 130, 410);
    ...
    case WM_PAINT: {
            ...
            // *** Gradient
            int r1 = 64, g1 = 58, b1 = 180;

            /* End Color; Change the R,G,B values
            to the color of your choice */
            int r2 = 100, g2 = 190, b2 = 255;

            /* loop to create the gradient */
            for(int i = rcGradient.left; i < rcGradient.right; i++)
            {
                int r,g,b;
                r = r1 + (i * abs((r2-r1)) / rcGradient.right);
                g = g1 + (i * abs((g2-g1)) / rcGradient.right);
                b = b1 + (i * abs((b2-b1)) / rcGradient.right);
                temp.left = i;
