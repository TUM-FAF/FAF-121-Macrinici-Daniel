...
int NrRings = 0;
Ring* B = new Ring[30];                                                // Keeps the information required to draw a Ring
RECT WorkingAreaRect;
int SpeedRed = 990;                                                    // The value that will be subtracted from 1000. Result will be the actual speed
// Double buffering
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    wcx.style = 0;//CS_DBLCLKS;                                         // Class styles
    ...
    hwndMain = CreateWindow(  "MainWindow",                         // Window class name
                              "Laboratory Work #4 Daniel Macrinici",                 // Window title
...
LRESULT CALLBACK MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch(msg)
    {
        ...
        case WM_CREATE:
        {
            ...
            // Setting up a timer
            SetTimer(hwnd, IDT_TIMER1, 50, (TIMERPROC) NULL); //setting the time lapse in which the velocity
            // of the frames is set up
        }
        break;
        ...
        case WM_LBUTTONDOWN:
        {
            ...
            else if (ClicksCounter == 1)
            {
                ...
                B[NrRings].Direction = (rand() % 358)+2; // direction from 2 to 360 degrees
                ClicksCounter++;
                InvalidateRect(hwnd, &WorkingAreaRect, FALSE);
            }
        }
        ...
        case WM_MOUSEMOVE:
        {
            ...
            if (ClicksCounter == 1)
            {
                ...
                if (max(xPos-B[NrRings].Coord.left, yPos-B[NrRings].Coord.top)>30.0
                    && max(xPos-B[NrRings].Coord.left, yPos-B[NrRings].Coord.top)<400.0)
                {
        ...
        case WM_PAINT:
        {
            ...
            FillRect(hdcMem, &WorkingAreaRect, (HBRUSH)GetStockObject(GRAY_BRUSH));
            ...
            for (int i=0; i<NrRings; i++)
            {
                ...
                HPEN hPen = CreatePen(PS_SOLID, 3, qColor);
...
void Initialization(void)
{
    ...
    WorkingAreaRect.bottom = 728;

    // Creating a pen for the working area rectangle frame
    COLORREF qColor = RGB(0, 0, 0);   // BLACK
    WndFramePen = CreatePen(PS_DASH, 0, qColor);
